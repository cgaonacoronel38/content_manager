...
public class DeviceContentFacade extends AbstractFacade<DeviceContent> {
    ...
    public ContentBean getContentBeanForDevice(String device) throws Exception {
        try {
            StringBuilder sb = new StringBuilder();

            sb.append("select * from content_handler.view_device_content ");
            sb.append(" where uuid_device like ?1 and active is true and downloaded is false limit 1");
            ...
            java.util.logging.Logger.getLogger(DeviceContentFacade.class.getName()).log(Level.SEVERE, null, ex);
            throw new Exception("Error al obtener contenido .", ex);
        }
    }
    ...
    public Integer confirmDownload(String device, String content) throws GDMEJBException {
        try {
            UUID uuidDevice = UUID.fromString(device);
            UUID uuidContent = UUID.fromString(content);

            StringBuilder sb = new StringBuilder();
            sb.append("update content_handler.device_content ");
            sb.append(" set downloaded = true, date_download = current_date ");
            sb.append(" where uuid_content = ?1 ");
            sb.append(" and uuid_device = ?2 ");

            Query q = getEntityManager().createNativeQuery(sb.toString());
            q.setParameter(1, uuidContent);
            q.setParameter(2, uuidDevice);

            return q.executeUpdate();
        } catch (Exception ex) {
            System.err.println("Error al registrar confirmacion de descarga "+ex.getMessage());
