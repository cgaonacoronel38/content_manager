...
public class WSContent implements Serializable {
    ...
    public Response downloadFile(@PathParam("content") String content) {
        try {
            String path = contentEJB.getContentPath(UUID.fromString(content));
            log.info("Path "+path);
    ...
    @GET
    @Path("/comfirmDownload/{device}/{content}")
    @Produces(MediaType.APPLICATION_JSON)
    public Response confirmDownload(@PathParam("device") String device, @PathParam("content") String content) {
        WSRespondeStatus responseStatus = WSRespondeStatus.REGISTER_ERROR;
        Integer update;
        try {
            update = deviceContentEJB.confirmDownload(device, content);
            if(update > 0){
